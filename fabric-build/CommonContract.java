...
public class CommonContract implements ContractInterface {
  ...
  public Boolean create(Context context, String type, String key, String value) {
    ...
    String hash = null;
    try {
      String ipfsRsStr = HttpClient.post("http://192.168.3.12:9094/add", value);
      IpfsRs ipfsRs = gson.fromJson(ipfsRsStr, IpfsRs.class);
      hash = ipfsRs.getCid().getHash();
      log.info(hash);
    } catch (IOException e) {
      e.printStackTrace();
    }
    Value value1 = gson.fromJson(value, Value.class);
    value1.getValues().setValue(hash);
    String value2 = gson.toJson(value1);
//    stub.putStringState(compositeKey.toString(), value);
    stub.putStringState(compositeKey.toString(), value2);
    try {
      Client.send("测试hash：" + hash);
    } catch (IOException e) {
      e.printStackTrace();
    }
    return Boolean.TRUE;
